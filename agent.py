...
def contains_devanagari(txt: str) -> bool:
    return bool(DEVANAGARI_RE.search(txt or ""))
...
def extract_text_any(uploaded_file) -> Tuple[str, List[str]]:
    ...
    try:
        ...
        if name.endswith(".pdf"):
            t, more = extract_text_from_pdf(data)
            return t, logs + more
        if any(name.endswith(ext) for ext in (".png", ".jpg", ".jpeg", ".webp", ".tif", ".tiff")):
            t, more = extract_text_from_image(data)
            return t, logs + more
...
with t3:
    ...
    references_text = st.text_area(
        "References (one per line) — e.g., GR number/date, letters, hearing date",
    ...
    if generate:
        if case_file is None or gr_file is None:
            ...
            if lang_choice in ["Marathi", "Both"]:
                order_mr = order_marathi_quasi(meta, decision, refs)
            ...
            if lang_choice in ["English", "Both"]:
                order_en = order_english_quasi(meta, decision, refs)
